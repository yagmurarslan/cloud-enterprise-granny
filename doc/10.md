...
> *Documentation is like sex: when it is good, it is very, very good; and when it is bad, it is better than nothing.*   -- Dick Brandon [[REF]](http://www.linfo.org/q_documentation.html)
...
| freemarkerXMLProcessingTemplate  | That's probably the most interesting attribute, since it allows to specifiy a custom [Freemarker](http://freemarker.org/) template to be used for the creation of the documentation. I usually modify the default template to a large extend to make teh documentation blend more nicely with the rest of the application. We'll focus on that topic in more detail in a separate chapter.  |
...
Finally, we add one more line to the `servlet-context.xml` to make sure that the documentation is served more efficently and not going through the whole process required to serve dynamic content. Here's the respective coding:
...
**PS:** The custom freemarket template will follow shorty in a seperate commit to provide more transparency into the changes I applied to the orignal source. 